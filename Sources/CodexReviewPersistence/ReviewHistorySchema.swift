...
enum ReviewHistorySchema {
    static func makeMigrator() -> DatabaseMigrator {
        ...
        migrator.registerMigration("v4_allow_known_previous_process_exit_end") { db in
            try #sql(
                """
                CREATE TABLE "review_records_v4" (
                  "id" TEXT NOT NULL PRIMARY KEY,
                  "cwd" TEXT NOT NULL REFERENCES "review_workspaces"("cwd") ON DELETE CASCADE,
                  "sortOrder" REAL NOT NULL,
            ...
            // GRDB owns deferred foreign-key validation for this migration. Leaving the child
            // table in place preserves finding rows while the parent table is replaced.
            try #sql("DROP TABLE \"review_records\"").execute(db)
            try #sql(
                """
                ALTER TABLE "review_records_v4"
                RENAME TO "review_records"
                """
            )
            .execute(db)
        ...
        migrator.registerMigration("v6_queue_review_starts") { db in
            try #sql(
                """
                CREATE TABLE "review_records_v6" (
                  "id" TEXT NOT NULL PRIMARY KEY,
                  "cwd" TEXT NOT NULL REFERENCES "review_workspaces"("cwd") ON DELETE CASCADE,
                  "sortOrder" REAL NOT NULL,
                  ...
                  CHECK (("phase" != 'queued' OR "startedAt" IS NULL)
                    AND ("phase" != 'active' OR "startedAt" IS NOT NULL)),
                  CHECK (COALESCE(
                    ("targetKind" = 'uncommittedChanges'
                      AND "targetBranch" IS NULL
                      AND "targetCommitSHA" IS NULL
            ...
            try #sql(
                """
                INSERT INTO "review_records_v6" (
                  "id", "cwd", "sortOrder",
                  "targetKind", "targetBranch", "targetCommitSHA", "targetCommitTitle",
                  "targetInstructions", "startedModel", "acceptedAt", "startedAt", "phase",
                  "terminalModel", "terminalKind", "interruptionKind",
                  "cancellationSource", "cancellationMessage", "terminalMessage", "endedAt",
                  "summary", "canonicalReview", "parsedState", "parsedFindingCount",
                  "parsedSource", "parserVersion", "terminalCommittedAt", "createdAt", "updatedAt", "reviewThreadID", "threadID"
                )
                SELECT
                  "id", "cwd", "sortOrder",
                  "targetKind", "targetBranch", "targetCommitSHA", "targetCommitTitle",
                  "targetInstructions", "startedModel", "startedAt", "startedAt", "phase",
                  "terminalModel", "terminalKind", "interruptionKind",
                  "cancellationSource", "cancellationMessage", "terminalMessage", "endedAt",
                  "summary", "canonicalReview", "parsedState", "parsedFindingCount",
                  "parsedSource", "parserVersion", "terminalCommittedAt", "createdAt", "updatedAt", "reviewThreadID", "threadID"
