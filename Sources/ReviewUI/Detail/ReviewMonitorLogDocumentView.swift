...
final class ReviewMonitorLogDocumentView: NSView, NSUserInterfaceValidations, @preconcurrency NSTextViewportLayoutControllerDelegate {
    ...
    private var wordFadeAnimations: [ReviewMonitorLog.WordFadeAnimation] = []
    ...
    private struct FinderStringMapping {
        var string: String
        var segments: [FinderStringSegment]
    }
    ...
    private struct CommandOutputPanelLayoutAnimation {
        var toggledBlockID: ReviewMonitorLog.BlockID
    ...
    var expandedCommandOutputBlockIDsForDisplayDocument: Set<ReviewMonitorLog.BlockID> {
    ...
    func applyPresentation(
        _ document: ReviewMonitorLog.Document,
        appended append: ReviewMonitorLog.Append? = nil,
    ...
    func updateFinderSupplement(_ document: ReviewMonitorLog.Document) {
    ...
    @discardableResult
    func toggleCommandOutputPanelExpansionState(_ blockID: ReviewMonitorLog.BlockID) -> Bool {
    ...
    private func changedCommandOutputPanelRanges(
        from previousPanels: [ReviewMonitorLog.CommandOutputPanel],
        to nextPanels: [ReviewMonitorLog.CommandOutputPanel]
    ) -> [NSRange] {
    ...
    private func firstCommandOutputPanelsByID(
        _ panels: [ReviewMonitorLog.CommandOutputPanel]
    ) -> [ReviewMonitorLog.BlockID: ReviewMonitorLog.CommandOutputPanel] {
        var panelsByID: [ReviewMonitorLog.BlockID: ReviewMonitorLog.CommandOutputPanel] = [:]
    ...
    private func displayPanelsWithCurrentExpansionState(
        _ panels: [ReviewMonitorLog.CommandOutputPanel]
    ) -> [ReviewMonitorLog.CommandOutputPanel] {
    ...
    private func refreshCommandOutputPanelAttachments(for panel: ReviewMonitorLog.CommandOutputPanel) {
        ...
        textContentStorage.performEditingTransaction {
            ...
            applyCommandOutputAttachments([panel], in: targetRange)
        }
    }
    ...
    private func styleInvalidationRange(
        for append: ReviewMonitorLog.Append,
    ...
    private func styleInvalidationRange(
        for replacement: ReviewMonitorLog.Replacement,
        in document: ReviewMonitorLog.Document
    ) -> NSRange? {
    ...
    private func attributes(for style: ReviewMonitorLog.TextStyle) -> [NSAttributedString.Key: Any] {
    ...
    private func planTextColor(for status: ReviewMonitorLog.PlanStatus?) -> NSColor {
        normalTextColor
    }
    ...
    private func commandOutputPanelLayoutAnimation(
        toggledBlockID: ReviewMonitorLog.BlockID
    ) -> CommandOutputPanelLayoutAnimation {
        ...
        return CommandOutputPanelLayoutAnimation(
    ...
    private func performCommandOutputPanelLayoutAnimation(_ animation: CommandOutputPanelLayoutAnimation) {
        guard shouldAnimateCommandOutputPanelTransitions else {
            return
        }

        let orderedFragmentViews = visibleFragmentViews.sorted { $0.frame.minY < $1.frame.minY }
        let panelViews = commandOutputPanelAttachmentViews()
    ...
    private func updateWordFadeAnimations(at now: TimeInterval) {
        ...
        var activeAnimations: [ReviewMonitorLog.WordFadeAnimation] = []
    ...
    func finderVisibleCharacterRanges() -> [NSValue] {
        let mapping = finderStringMapping()
        let visibleDocumentRanges = visibleCharacterRanges().map(\.rangeValue)
        guard visibleDocumentRanges.isEmpty == false else {
            return []
        }
    ...
    func documentRangeForFinderRange(_ range: NSRange) -> NSRange? {
        firstDocumentRange(for: range, in: finderStringMapping())
    }

    func finderRangeForDocumentRange(_ range: NSRange) -> NSRange {
        firstFinderRange(forDocumentRange: range, in: finderStringMapping()) ?? range
    }
    ...
    func rects(forFinderCharacterRange range: NSRange) -> [NSValue] {
        let mapping = finderStringMapping()
        var rects: [NSValue] = []
        for segment in mapping.segments {
    ...
    func drawCharacters(inFinderRange range: NSRange, forContentView view: NSView) {
        let mapping = finderStringMapping()
        for segment in mapping.segments {
            let intersection = NSIntersectionRange(segment.finderRange, range)
            guard intersection.length > 0 else {
    ...
    private func finderStringMapping() -> FinderStringMapping {
        if let cachedFinderStringMapping {
            return cachedFinderStringMapping
        }

        let documentString = textStorage.string as NSString
        var finderString = ""
        var finderUTF16Length = 0
        var segments: [FinderStringSegment] = []

        func append(_ text: String) -> NSRange {
            let range = NSRange(location: finderUTF16Length, length: (text as NSString).length)
            finderString += text
            finderUTF16Length += range.length
            return range
        }
    ...
    private func invalidateFinderStringMapping() {
        cachedFinderStringMapping = nil
    }

    private func mapFinderRange(
        _ range: NSRange,
    ...
    private func firstDocumentRange(
        for finderRange: NSRange,
        in mapping: FinderStringMapping
    ) -> NSRange? {
        for segment in mapping.segments {
            let intersection = NSIntersectionRange(segment.finderRange, finderRange)
            guard intersection.length > 0 else {
                continue
            }
    ...
    private func updateDecorationRects() {
        ...
        let resolved = visibleDecorations.compactMap { decoration -> ReviewMonitorLog.ResolvedDecoration? in
            ...
            return ReviewMonitorLog.ResolvedDecoration(style: decoration.style, rect: rect)
        }
    ...
    private func decorationNeedsResolvedRect(_ style: ReviewMonitorLog.DecorationStyle) -> Bool {
        switch style {
