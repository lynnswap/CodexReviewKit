...
struct ReviewUITests {
    ...
    @Test func commandOutputRendersCollapsedTextKitPanelAndExpandsInline() async throws {
        ...
        let commandRange = (transport.displayedLogForTesting as NSString).range(of: "$ swift test")
        try #require(commandRange.location != NSNotFound)
        #expect(transport.logHitTestTargetsDocumentViewForFirstOccurrenceForTesting("$ swift test"))
        transport.setSelectedLogRangeForTesting(commandRange)
